...
#'
#' @details The function checks if the two given k-partitions are equivalent, i.e. if their groupings are the same (useful in the case that the partitions have a divergent group numbering).
#'
...
equivalent_partition <- function(p1,p2) {
    ...
    if (nrow(unique(cbind(p1,p2)))==length(unique(p1))) {res <- TRUE} else {res <- FALSE}
    return(res)
  }
