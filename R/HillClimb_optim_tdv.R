...
#'
#' @title TotDiffVal1 optimization using hill climbing algorithms
#'
#' @description This function searches for partitions of the columns of a given matrix, optimizing the TotDiffVal1 index.
#'
...
#' @export
#'
HillClimb_optim_tdv <- function(m, k, maxit=10, p.ini="random", min.g.size=2, random.first=FALSE, rf.neigh.size=1, rf.maxit=500) {
  t <- Sys.time()
  ...
  if ((mgs <- as.integer(min.g.size))<2) {stop("Object min.g.size must be greater than 2")}
  mt <- t(m)
  ...
  if (p.ini[1] == "random") {p.ini <- sample(c(rep(1:k, mgs), sample(k,nr-mgs*k,replace=TRUE))); tp <- table(p.ini)} else {
    tp <- table(p.ini)
  ...
  max.tdv.neighbour <- function (p) {
    ...
    mat.neig.tdv <- sapply(1:nrow(mat.neig$p.list), function (x) { #as it is difficult to parallelize!
  ...
  #HILL CLIMBING
  if (maxit==0) {res <- NULL; parcor=p.ini; loc_max=NULL} else {
    loc_max <- FALSE
    parcor <- p.ini
